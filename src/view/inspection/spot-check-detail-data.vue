      ...
      <table class="detail-con-tab" border="1" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td colspan="10" class="detail-content">
            {{spotCheckData.producer}}
            <span style="font-size: 12px; color: #a7a5a5;">&nbsp;&nbsp;&nbsp;&nbsp;该企业共有{{count.total}}条抽检数据，合格{{count.hgCount}}条，不合格{{count.bhgCount}}条。</span>
          </td>
        </tr>
        <tr>
...
export default {
  ...
  methods: {
    ...
    getSpotCheckById () {
      ...
      axios.request(option).then(res => {
        if (res.data.code === 200) {
          ...
          _this.getCountByProducer(_this.spotCheckData.producer)
          global.initWebTitle(_this.spotCheckData.sample + '-抽检数据-叶竹洪博客')
        }
      })
    },
    handleSearch () {
    ...
    getCountByProducer (producer) {
      const _this = this
      const option = {
        url: '/api/spotCheck/getCountByProducer?producer=' + producer,
        method: 'get'
      }
      axios.request(option).then(res => {
        let count = res.data.data
        _this.count.total = count.total
        _this.count.hgCount = count.hgCount
        _this.count.bhgCount = count.bhgCount
      })
    }
  }
}
