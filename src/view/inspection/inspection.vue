<style scoped>
  ...
  .ul-up li.active_class{
    background-color: #f3f9fc;
  }
  .ul-down li.active_class{
    background-color: #f3f9fc;
  }
  .menu-ul li:hover{
    color: #0b81bf;
    ...
    <Content>
      <Layout class="main-layout-con">
        <Sider hide-trigger style="background-color: #f5f7f9; margin-right: 15px;" class="sider">
          ...
          <div style="border: 1px solid #dcdee2; margin-bottom: 15px;" v-if="type === 'CC'">
            <div class="menu-title">
              <Icon type="md-chatbubbles" />
              {{leftDownData.title}}
            </div>
            <ul class="menu-ul ul-down">
...
export default {
  ...
  methods: {
    ...
    toRouter (option, typeCode, idx, val) {
      ...
      if (option === 'up') {
        if (this.type === 'CC') {
          this.$store.dispatch('CreateParam', { type: this.type, query: [{ key: typeCode.split('_')[1], value: val }, { key: 'type', value: '' }] })
        }
      ...
      // console.log(this.upActiveIdx + '***' + this.downActiveIdx)
    ...
    initWebTitle () {
      ...
      let titlePrefix = ''
      if (_this.type === 'SC') {
        titlePrefix = _this.upActiveIdx === null ? '抽检数据' : _this.leftUpData.menuList[_this.upActiveIdx].label + '-抽检数据'
      }
      if (_this.type === 'CC') {
        titlePrefix = _this.upActiveIdx === null ? (_this.downActiveIdx === null ? '标准' : _this.leftDownData.menuList[_this.downActiveIdx].label + '-标准') : _this.leftUpData.menuList[_this.upActiveIdx].label + '-标准'
      }
      if (_this.type === 'LW') {
        titlePrefix = _this.upActiveIdx === null ? '法规' : _this.leftUpData.menuList[_this.upActiveIdx].label + '-法规'
      }
      if (_this.type === 'FC') {
        titlePrefix = _this.upActiveIdx === null ? '监督检查' : _this.leftUpData.menuList[_this.upActiveIdx].label + '-监督检查'
      }
      if (_this.type === 'AC') {
        titlePrefix = _this.upActiveIdx === null ? '文章' : _this.leftUpData.menuList[_this.upActiveIdx].label + '-文章'
      }
      global.initWebTitle(titlePrefix + '-叶竹洪博客')
    }
  }
}
</script>
