  ...
  <div>
    ...
    <div class="detail-data detail-con">
      ...
      <table class="detail-con-tab" border="1" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td colspan="10" class="detail-content">
            {{flightCheckData.businessName}}
            <span style="font-size: 12px; color: #a7a5a5;">&nbsp;&nbsp;&nbsp;&nbsp;该企业共有{{count.total}}条监督检查数据，有缺陷{{count.hasCount}}次，无缺陷{{count.noCount}}次。</span>
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td colspan="10" class="detail-content" v-html="flightCheckData.problem" style="text-align:justify;"></td>
      ...
      <div>
        ...
        <div class="data-detail-about data-detail-about-right">
          <div class="data-detail-about-title">
...
export default {
  ...
  methods: {
    ...
    getFlightCheckById () {
      ...
      axios.request(option).then(res => {
        if (res.data.code === 200) {
          ...
          _this.getCountByBusinessName(_this.flightCheckData.businessName)
          global.initWebTitle(_this.flightCheckData.businessName + '-监督检查-叶竹洪博客')
        }
      })
    },
    handleSearch () {
    ...
    getCountByBusinessName (businessName) {
      const _this = this
      const option = {
        url: '/api/flightCheck/getCountByBusinessName?businessName=' + businessName,
        method: 'get'
      }
      axios.request(option).then(res => {
        let count = res.data.data
        _this.count.total = count.total
        _this.count.hasCount = count.hasCount
        _this.count.noCount = count.noCount
      })
    }
  }
}
