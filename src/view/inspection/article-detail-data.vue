  ...
  <div>
    ...
    <div class="detail-data">
      <Row style="border-bottom: 1px solid #e5e5e5;">
        <i-col span="24" class="detail-data-title">{{articleData.title}}</i-col>
      </Row>
      <Row>
        <i-col span="6" class="article_font">文章类型：{{articleData.typeName}}</i-col>
        <i-col span="6" offset="1" class="article_font">阅读量：{{articleData.readCount}}</i-col>
      </Row>
      <Row>
        <i-col span="6" class="article_font">来源：{{articleData.author}}</i-col>
        <i-col span="6" offset="1" class="article_font">发布时间：{{articleData.publishTime}}</i-col>
      </Row>
      <Row style="border-bottom: 1px solid #e5e5e5;">
        <i-col span="24" class="article_font">附件：
      ...
      <Row>
        <i-col span="24" class="detail-data-content" v-html="articleData.content"></i-col>
      </Row>
    </div>
  </div>
</template>
    ...
    getArticleById () {
      ...
      axios.request(option).then(res => {
        if (res.data.code === 200) {
          ...
          global.initWebTitle(_this.articleData.title + '-文章-叶竹洪博客')
        }
      })
    },
    handleClear (e) {
